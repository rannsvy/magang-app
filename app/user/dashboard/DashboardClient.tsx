...
type Job = {
  ...
  // opsional dari BE (untuk branding)
...
function stripPlateParens(text: string) {
  return (text ?? "")
    .replace(/[()]/g, "")
    .replace(/\s{2,}/g, " ")
    .trim();
...
function milestoneMessage(ms: Milestone): string {
  switch (ms) {
    ...
    case 75:
      return "Wahh hampir selesai nih, gas terus dikit lagi bisa!";
    case 100:
      return "Akhirnya selesai! Terima kasih atas kerja kerasnya!";
  }
}
    ...
    return new Promise((resolve) => {
      ...
      const t = setInterval(() => {
        if (typeof window.confetti === "function" || tries > 40) {
          clearInterval(t);
          resolve(
            typeof window.confetti === "function" ? window.confetti! : null
          );
        }
        tries++;
      }, 100);
...
export default function TechnicianDashboard() {
  ...
  // Border berdasarkan status (branding pakai background warna instansi)
  ...
  /** ==== Komponen Kartu (pakai branding + milestone) ==== */
  function JobCard({
    job,
    onClick,
  }: {
    ...
    const vehicleList: string[] = (
      job.vehicle_names?.length
        ...
        ? [job.vehicle_name]
        : []
    ).map(stripPlateParens) as string[];
        ...
        <CardContent className="px-2 py-1">
          <div className="flex justify-between items-start mb-1">
            <div className="flex-1 pr-2">
              <div
                className={`text-[10px] font-medium leading-none mb-0.5 ${monoText}`}
              >
                {instansi}
              </div>

              <h3
                className={`font-bold text-sm mb-0.5 leading-tight ${strongText}`}
              >
                {job.name}
              </h3>
              ...
              {job.type === "survey" && job.building_name ? (
                <p
                  className={`text-xs font-medium leading-tight mb-0.5 ${subtleText}`}
                >
            ...
            <div className="flex flex-col items-end gap-0.5">
              ...
              <div className="flex items-center gap-1">
                <div
                  ...
                  title={
                    badge.countText ? `Progress ${badge.countText}` : undefined
                  }
                >
                  {badge.text}
                </div>
              ...
              {(job.supervisor_name || job.sales_name) && (
                <div
                  className={`text-[10px] leading-tight text-right mt-0.5 ${subtleText}`}
                >
                  <div>
                    SPV:{" "}
                    <b className={strongText}>{job.supervisor_name ?? "-"}</b>
                  </div>
                  <div>
