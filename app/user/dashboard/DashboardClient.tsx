...
/** ===================== Milestone Celebration (LOCAL UTIL) ===================== **/
...
function getReachedMilestone(progressPct: number): Milestone | null {
  ...
  if (pct === 100) return 100; // 100% harus tepat 100
  if (pct >= 75) return 75;
  if (pct >= 50) return 50;
  if (pct >= 25) return 25;
  return null;
...
function getStatusDisplay(job: Job) {
  ...
  const countText = hasCount ? `${job.progressDone}/${job.progressTotal}` : null;
...
function getCardBorder(job: Job) {
...
function JobCard({
  ...
  // Trigger milestone di level card (aman untuk aturan hooks)
...
export default function TechnicianDashboard() {
  ...
  const surveyRoomsChannelRef = useRef<ReturnType<typeof supabase.channel> | null>(
    null
  );
  ...
  async function attachProgress(items: Job[]): Promise<Job[]> {
    const enriched = await Promise.all(
      items.map(async (j) => {
        const { percent, isPending, done, total } = await getJobProgress(j.job_id);
