...
import { type OcrInfo, recognizeSerialNumber, recognizeCableMeters } from "@/lib/ocr"
...
interface PhotoCategory {
  ...
  snDraft?: string
  meter?: number
  /** Token unik untuk mengikat hasil OCR ke foto yang benar */
  photoToken?: number
}
...
export default function UploadFotoPage() {
  ...
  // === RESET SN saat pilih foto baru ===
  const handlePhotoCapture = (categoryId: string, event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (!file) return

    // Reset SN & OCR ketika ambil ulang foto kategori yang butuh SN
    const cat = categories.find(c => c.id === categoryId)
    if (cat?.requiresSerialNumber) {
      setCategories(prev =>
        prev.map(c =>
          c.id === categoryId ? { ...c, serialNumber: undefined, snDraft: undefined } : c
        )
      )
      setOcr(prev => ({ ...prev, [categoryId]: { status: "idle", progress: 0 } }))
    }
  ...
  async function runOCRForCategory_SN(catId: string, imageSource: Blob | string, token: number) {
    ...
    // TANPA paksa 8 char — gunakan hasil dari lib (sudah dinormalisasi dinamis)
    const finalSN = (serial || "").toUpperCase()

    if (finalSN) {
      setCategories((prev) =>
        prev.map((c) =>
          c.id === catId && c.photoToken === token
            ? { ...c, serialNumber: finalSN, snDraft: undefined }
            : c
        )
      )
    }
  ...
  async function runOCRForCategory_Cable(catId: string, imageSource: Blob | string, token: number) {
    const meter = await recognizeCableMeters(imageSource, {
      onProgress: (info) => setOcr((prev) => ({ ...prev, [catId]: info })),
    })
    setCategories((prev) =>
      prev.map((c) =>
        c.id === catId && c.photoToken === token ? { ...c, meter: meter ?? undefined } : c
      )
    )
  }

  // === Simpan Crop: kosongkan SN dulu, set token foto, lalu OCR ===
  const handleConfirmCrop = async () => {
    ...
    // Set foto + reset SN + set photoToken (penting untuk cegah race condition)
    setCategories((prev) =>
      prev.map((c) =>
        c.id === pendingCategoryId
          ? { ...c, photo: dataUrl, serialNumber: undefined, snDraft: undefined, photoToken: token }
          : c
      )
    )
    ...
    if (cat) {
      if (cat.requiresSerialNumber) {
        await runOCRForCategory_SN(pendingCategoryId, blob, token)
      } else if (isCableCategory(cat.name)) {
        await runOCRForCategory_Cable(pendingCategoryId, blob, token)
      }
    }
  ...
  const handleConfirmSerialNumber = (categoryId: string) => {
    setCategories((prev) =>
      prev.map((cat) =>
        cat.id === categoryId
          ? { ...cat, serialNumber: (cat.snDraft || "").trim().toUpperCase(), snDraft: undefined }
          : cat
      )
    )
            ...
            {currentCategories.map((category) => {
              ...
              return (
                <div key={category.id} className="space-y-1">
                  ...
                  {/* Hidden file input */}
                  <input
                    ref={(el) => { fileInputRefs.current[category.id] = el || null }}
                    type="file"
                    accept="image/*"
                    capture="environment"
                    className="hidden"
