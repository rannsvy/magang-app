...
export default function UploadFotoClient() {
  ...
  // kalau tidak ada query, coba dari localStorage (agar bisa buka upload di offline)
  useEffect(() => {
    if (!qJob) {
      try {
  ...
  // === KUNCI: merge pending local (queued/uploading) ke hasil refetch server ===
  const mergePending = (serverItems: PhotoCategory[], localItems: PhotoCategory[]) => {
    const mapPending = new Map<string, PhotoCategory>(
      localItems
        .filter((c) => c.uploadState === "queued" || c.uploadState === "uploading" || c.uploadState === "error")
        .map((c) => [c.id, c])
    );
    return serverItems.map((it) => {
      const p = mapPending.get(it.id);
      if (!p) return it;
      return {
        ...it,
        // pertahankan tampilan lokal sampai server update masuk
        photoThumb: p.photoThumb ?? it.photoThumb,
        uploadState: p.uploadState,
        queueId: p.queueId,
        uploadError: p.uploadError,
        photoToken: p.photoToken ?? it.photoToken,
        // kalau SN manual sudah diisi sebelum upload, jangan hilang
        serialNumber: it.serialNumber ?? p.serialNumber,
      };
    });
  };

  // snapshot untuk merge saat refetch
  const localSnapshotRef = useRef<PhotoCategory[]>([]);
  useEffect(() => {
    localSnapshotRef.current = categories;
  }, [categories]);
  ...
  // load dari cache + server (merge pending)
  useEffect(() => {
    if (!jobId) return;

    (async () => {
      // 1) Tampilkan cache dulu ketika offline
      if (!online && cacheKey) {
        try {
          const cached = localStorage.getItem(cacheKey);
          if (cached) setCategories(JSON.parse(cached) as PhotoCategory[]);
        } catch {}
      }

      // 2) Refetch server (kalau offline akan error → diamkan)
      try {
        const server = await fetchCategories(jobId);
        setCategories((prev) => {
          const merged = mergePending(server, prev);
          if (cacheKey) try { localStorage.setItem(cacheKey, JSON.stringify(merged)); } catch {}
          return merged;
        });
      } catch (e) {
        // Biarkan isi dari cache
      }
    })();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [jobId, online]);

  // realtime subscribe → saat server benar2 update, UI ikut
  useEffect(() => {
    if (!jobId) return;
  ...
  // === DENGAR pesan dari SW: { type: "sync-complete", queueIds: string[] }
  useEffect(() => {
    function onMessage(e: MessageEvent) {
      ...
      if (d?.type === "sync-complete" && Array.isArray(d.queueIds)) {
        ...
        // ambil ulang dari server supaya URL final masuk
        if (jobId) {
  ...
  // === Paksa SW replay saat kembali online
  useEffect(() => {
    if (online && typeof navigator !== "undefined" && navigator.serviceWorker?.controller) {
  ...
  const handlePhotoCapture = (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (cat?.requiresSerialNumber) {
      setCategories((prev) =>
        prev.map((c) => (c.id === id ? { ...c, serialNumber: undefined, snDraft: undefined } : c))
      );
  ...
  const handleConfirmCrop = async () => {
    ...
    try {
      ...
      if (cat?.serialNumber) fd.append("serialNumber", cat.serialNumber);
      ...
      if (result?.status === "uploaded") {
        ...
        setCategories((prev) =>
          prev.map((c) =>
            c.id === pendingCategoryId ? { ...c, uploadState: "queued", queueId: result.queueId as string, uploadError: undefined } : c
          )
        );
      } else {
        ...
        setCategories((prev) =>
          prev.map((c) => (c.id === pendingCategoryId ? { ...c, uploadState: "error", uploadError: msg } : c))
        );
      }
    } catch {
      setCategories((prev) =>
        ...
        <div className="max-w-4xl mx-auto">
          {!jobId ? (
            ...
            <>
                ...
                {slice.map((category) => {
                  ...
                  return (
                    <div key={category.id} className="space-y-1">
                      <Card className={`cursor-pointer transition-all hover:shadow-md ${styles} max-w-[110px] mx-auto`} onClick={() => handleCameraClick(category.id)}>
                        <CardContent className="p-1 flex items-center justify-center h-[50px] w-[110px] relative">
                          {category.photoThumb ? (
                            <img src={category.photoThumb} alt={category.name} className="max-w-full max-h-full object-contain rounded" loading="lazy" decoding="async" width={130} height={80} />
                          ) : category.photo ? (
                            <img src={category.photo} alt={category.name} className="max-w-full max-h-full object-contain rounded" loading="lazy" decoding="async" width={130} height={80} />
                          ) : (
                      ...
                      {category.uploadState && (
                        <p className="text-[10px] text-center text-gray-600">
                          ...
                          {category.uploadState === "error" && (
                            <span className="text-red-600">Gagal{category.uploadError ? `: ${category.uploadError}` : ""} — periksa koneksi/akses.</span>
                          )}
                        </p>
                      )}

                      {/* Kabel: panjang manual */}
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-1">
                                  <Input
                                    ...
                                    className="text-[10px]"
                                  />
                                  <Button type="button" className="h-6 px-2 text-[10px]" onClick={() => handleConfirmSerialNumber(category.id)} disabled={!((category.snDraft ?? "").trim().length)}>
                                    Enter
                                  </Button>
                                </div>
                              </div>
              ...
              <div className="mb-6">
                <Pagination currentPage={currentPage} totalPages={totalPages} onPrevPage={() => currentPage > 1 && setCurrentPage((p) => p - 1)} onNextPage={() => currentPage < totalPages && setCurrentPage((p) => p + 1)} />
              </div>
            </>
          )}
        </div>
          ...
          <div className={`bg-white rounded-xl p-4 w-[92vw] ${isPortrait ? "max-w-[480px]" : "max-w-[720px]"}`}>
            ...
            <div className="relative max-h-[70vh] max-w-[92vw] bg-black/5 rounded overflow-hidden flex items-center justify-center">
              <ReactCrop
                ...
                keepSelection
              >
                <img ref={imgRef} src={srcToCrop} alt="To crop" onLoad={(e) => onImageLoaded(e.currentTarget)} className="max-h/[70vh] max-w-[92vw] w-auto h-auto object-contain" />
              </ReactCrop>
            </div>
            ...
            <div className="mt-3 grid grid-cols-1 gap-3">
              ...
              {isPendingCable && (
                <div className="flex items-center gap-2">
                  ...
                  <input type="number" inputMode="decimal" step="0.1" min="0" placeholder="mis. 56" value={cableMeterDraft} onChange={(e) => setCableMeterDraft(e.target.value)} className="text-xs border rounded px-2 py-1 w-[140px]" />
                </div>
              )}
            </div>
