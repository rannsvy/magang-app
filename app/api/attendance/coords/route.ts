...
export async function GET(req: Request) {
  try {
    ...
    if (!projectId && jobId) {
      ...
      if (!proj?.id) {
        return NextResponse.json({ error: "jobId tidak ditemukan." }, { status: 404 });
      }
    ...
    const source =
      typeof r?.check_out_longitude === "number" || typeof r?.check_out_latitude === "number"
        ? "check_out"
        : typeof r?.check_in_longitude === "number" || typeof r?.check_in_latitude === "number"
        ? "check_in"
        : null;
