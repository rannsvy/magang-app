// components/admin/manage-teknisi/utils.ts
import type {
  FormUser,
  RoleFilter,
  StatusType,
...
export function buildTechPayloadFromForm(f: FormUser) {
  return {
    nama_panggilan: f.nama_panggilan || null,
    nama_lengkap: f.nama_lengkap || null,
    email: f.email || null,
    phone: f.phone || null,
    is_active: f.is_active === "true",
    inisial: f.inisial ? f.inisial.toUpperCase().slice(0, 2) : null,
  };
}

export function buildUserPayloadFromForm(f: FormUser) {
  return {
    nama_panggilan: f.nama_panggilan || null,
    nama_lengkap: f.nama_lengkap || null,
    email: f.email || null,
    ...
    is_active: f.is_active === "true",
    role_key: f.role,
    ...(f.role === "teknisi" && f.inisial
      ? { inisial: f.inisial.toUpperCase().slice(0, 2) }
      : {}),
  };
}
