...
type WaitlistUnit = {
  ...
  project_packages?: Array<{ seq: number; rw: string | null; rt: string | null }>;
  paket_count?: number;
};
...
export default function ManageProject({ onDone }: { onDone?: () => void }) {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await apiFetch(`/api/manage-projects?query=${encodeURIComponent(q)}`);
  ...
  async function handleSave() {
    const targets = Object.keys(selected).filter((id) => selected[id] && dates[id]);
    ...
    try {
      await Promise.all(
        targets.map((id) => {
          ...
          if (!iso) {
            throw new Error("Format tanggal tidak valid. Gunakan dd/mm/yyyy atau yyyy-mm-dd.");
          }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        />
        <Button onClick={load} disabled={loading}>Cari</Button>
      </div>
                  ...
                  <tr key={it.id} className="border-t align-top">
                    <td className="px-3 py-2">
                      ...
                      <div className="text-xs text-gray-500">
                        Sales: {it.sales_name ?? "—"} • Presales: {it.presales_name ?? "—"}
                      </div>
                      <div className="text-xs text-gray-500">
                        SPK: {it.tgl_spk_user ?? "—"} • PO: {it.tgl_terima_po ?? "—"}
                      </div>
                    </td>
